  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!shop) {
        ...
        // productData.category = {id:1,name:"camputer",subtitles:null}
...
router.delete(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      if (!product) {
        return next(new ErrorHandler(" Promo Product not found with this id!", 500));
      }
...
const productUpdated = await Product.findByIdAndUpdate( req.params.id , { $set: productUpdate } , {
  new: true,
  runValidators: true,
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      const files = req.files;
      // const imageUrls = files.map((file) => `${file.filename}`);
      const selecteProduct = await Product.findOne({ _id: req.params.id });
     const arrayImages = files.map((file) => `${file.filename}`);
