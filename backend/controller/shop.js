...
router.post("/create-shop", upload.single("file"), async (req, res, next) => {
  try {
    ...
    let newSeller = await Shop.findOne({ email : req.body.email });
    ...
    try {
      ...
      //           message: `Hello ${seller.name}, Marhba bik f Ecommerce connect`,
...
router.put(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      const { name, description, address, phoneNumber, zipCode , country,
        accountManager,
        otherShop,
        city,
        companyId,
...
router.put(
  ...
  //     });   
      // }else{
      //   console.log("resona");
      // // const existsfileCompanyId = await Shop.findById(req.seller._id);
      // const filename = req.file.filename;
      // const filePath = `uploads/${filename}`;
      // // const existFileCompanyIdPath = `uploads/${fileCompanyId}`;
      // fs.unlink(filePath, (err) => {
      //   if (err) {
      //     console.log(err);
      //     res.status(500).json({ message: "Error deleting file" });
      ...
      // }
      const fileUrl = path.join(req.file.filename);
      // if (existFileCompanyIdPath) {
      //   fs.unlinkSync(existFileCompanyIdPath);
      // }
      // if (!existFileCompanyIdPath) {
        // fs.unlink(filePath);
      // }
